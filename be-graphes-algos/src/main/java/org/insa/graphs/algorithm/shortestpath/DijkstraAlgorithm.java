...
public class DijkstraAlgorithm extends ShortestPathAlgorithm {
    ...
    protected ShortestPathSolution doRun() {
        ...
        for (Node n : nodes) {
        	Label newLabel =creerLabel(n,data.getDestination(),data);
        	...
        	if(x ==labels.get(data.getDestination().getId())) {
	        	...
	        	for(Arc a : graph.get(x.sommet_courant).getSuccessors()) { //on parcourt tous les y sucesseurs de x
	        		...
	        		if (!(y.marque))  { //si y n'est pas marqué
	        			...
	        			y.cout = Math.min(y.getCost(), x.getCost() + (float)data.getCost(a));
        ...
        if (!alafin) {
        	...
        	if (data.getMode()== Mode.LENGTH) {
        		...
        		solution = new ShortestPathSolution(data,Status.INFEASIBLE,Path.createFastestPathFromNodes(graph, Node_solu));
        	}
	        ...
	        */
	        Path newPath;
	        if (data.getMode()== Mode.LENGTH) {
	        	newPath = Path.createShortestPathFromNodes(graph, Node_solu);//new Path(graph, listArcsSolution);
	        }
	        else {
	        	newPath = Path.createFastestPathFromNodes(graph, Node_solu);
	        }
